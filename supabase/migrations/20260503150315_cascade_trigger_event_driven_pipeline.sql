-- Cascade pipeline: triggers fire edge functions instead of cron jobs
-- Lead INSERT          -> hunter-linkedin-enrich?id=N
-- email IS NOT NULL    -> lead-fit-scorer?id=N
-- fit_score >= 70      -> first-touch-drafter?id=N
-- email_approved=true  -> outbound-sender?id=N
--
-- All edge calls are async (pg_net.http_get) -- they don't block the transaction.
-- Failures are logged into outbound_events.properties.cascade_dispatch_status for observability.
...
BEGIN
  ...
  -- ============================================================
  -- INSERT: log + fire enrichment cascade
  -- ============================================================
  IF TG_OP = 'INSERT' THEN
    ...
    -- CASCADE: if lead has linkedin_url and no email yet, fire hunter enricher
    IF NEW.linkedin_url IS NOT NULL
      ...
      EXCEPTION WHEN OTHERS THEN
        -- Don't block the insert; just log it
  ...
  -- ============================================================
  -- UPDATE: state-change events + cascading dispatches
  -- ============================================================

  -- Email found (was NULL, now set) -> log event AND fire fit scorer
  ...
  -- Fit-scoring event AND cascade to drafter if score >= 70
  IF NEW.fit_score IS NOT NULL AND OLD.fit_score IS NULL THEN
    ...
    -- CASCADE: high-fit lead -> fire drafter
    IF NEW.fit_score >= 70
       AND COALESCE(NEW.automation_paused, false) = false
       AND COALESCE(NEW.is_paying_customer, false) = false
  ...
  -- Email approved (was false, now true) -> CASCADE to sender
