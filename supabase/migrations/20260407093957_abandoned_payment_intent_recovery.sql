...
-- Function: push abandoned payment intent to MailerLite nurture
...
DECLARE
  ...
  v_payload jsonb;
BEGIN
  -- Find payment intents created 1-24 hours ago that never succeeded
  FOR rec IN
  ...
  LOOP
    ...
    -- Skip if already a buyer
    IF EXISTS (SELECT 1 FROM public.purchases WHERE email = v_email) THEN
      CONTINUE;
    END IF;

    -- Skip if already in outreach_log for this payment intent
    IF EXISTS (SELECT 1 FROM public.outreach_log WHERE email = v_email AND trigger_reason = 'abandoned_payment') THEN
      CONTINUE;
    END IF;

    -- Build MailerLite payload — group 181203426870298094 is Assessment Leads
...
-- Schedule it to run every hour via pg_cron
