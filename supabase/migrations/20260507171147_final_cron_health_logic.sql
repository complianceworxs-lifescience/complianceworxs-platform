-- Only flag crons whose schedule starts with */ (every N minutes) OR has a comma in the hour field (multiple times/day).
-- These are the only crons we expect to see fire within a 6h window.
...
crons AS (
  -- Only flag genuinely high-frequency crons.
  SELECT count(*) AS broken_crons
  FROM cron.job j
  WHERE j.active = true
    AND (
      j.schedule LIKE '*/%'                          -- every N minutes (*/5, */15, */30)
      OR split_part(j.schedule,' ',2) LIKE '%,%'     -- comma in hour field (multi-times/day)
      OR split_part(j.schedule,' ',2) LIKE '%-%'     -- range in hour field (e.g. 9-17)
    )
    AND EXISTS (SELECT 1 FROM cron.job_run_details d WHERE d.jobid = j.jobid)
    AND NOT EXISTS (SELECT 1 FROM cron.job_run_details d WHERE d.jobid = j.jobid AND d.start_time > NOW() - INTERVAL '2 hours' AND d.status = 'succeeded')
),
phantoms AS (
...
SELECT 
  CONCAT(
    ...
    CASE WHEN crons.broken_crons > 0 THEN '⚠ ' || crons.broken_crons || E' high-freq crons not succeeded in 2h\n' ELSE '' END,
