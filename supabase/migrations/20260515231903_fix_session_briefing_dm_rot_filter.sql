-- Fix: briefing's "Unactioned DM replies" filter doesn't respect followup_completed_at
-- Currently flags any contact with dm_replied_at IS NOT NULL AND automation_paused = FALSE
-- This means contacts who replied but were actioned still flag as rot forever
-- Fix: exclude contacts with followup_completed_at set
...
WITH revenue AS (
  ...
  -- FIX: exclude contacts where followup has been completed
  SELECT count(*) AS count,
  ...
  WHERE warm_outbound_staging.dm_replied_at IS NOT NULL 
    ...
    AND warm_outbound_staging.followup_completed_at IS NULL  -- THIS IS THE FIX
