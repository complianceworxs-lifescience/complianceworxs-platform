...
SELECT
  ...
  CASE
    ...
    WHEN leads_sent >= 10 AND pct_reply_rate >= 10 THEN 'SCALE'
  ...
  CASE
    ...
    WHEN leads_replied >= 5 
      THEN leads_replied || ' replies on ' || leads_sent || ' sent (' || COALESCE(pct_reply_rate::text, '0') || '%) - clone this search and feed it more.'
    WHEN leads_sent >= 10 AND pct_reply_rate >= 10 
      THEN 'Reply rate ' || pct_reply_rate || '% on ' || leads_sent || ' sent - top performer. Clone the search parameters.'
...
ORDER BY 
  CASE
    ...
    WHEN leads_sent >= 10 AND pct_reply_rate >= 10 THEN 1
...
SELECT 'views v2 deployed' AS status;