-- Update the cascade trigger to skip disqualified rows on INSERT.
-- Saves Hunter credits by not firing enrichment for rows that are pre-flagged as junk.
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    -- CASCADE 1: Hunter enrichment - skip if pre-disqualified or already failed
    IF NEW.linkedin_url IS NOT NULL
       AND NEW.email IS NULL
       AND COALESCE(NEW.enrichment_status, 'pending') = 'pending'  -- NEW: skip junk/disqualified
    ...
    -- CASCADE 2: Attio push - skip if pre-disqualified to keep Attio clean
    IF NEW.linkedin_url IS NOT NULL
       AND NEW.full_name IS NOT NULL
       AND NEW.attio_record_id IS NULL
       AND COALESCE(NEW.enrichment_status, 'pending') NOT LIKE 'disqualified%'  -- NEW
       AND COALESCE(NEW.automation_paused, false) = false
       AND COALESCE(NEW.is_paying_customer, false) = false THEN
      BEGIN
        SELECT net.http_get(
  ...
  -- UPDATE branch (unchanged from previous)
