-- Rewrite session_start_briefing to include a RECONSTRUCTION FUNNEL section.
-- The /reconstruction page is the second-best converting surface on the site
-- (75 uniques → 13 completers → 9 routed click-throughs in 7d). Previously
-- invisible to the briefing; now surfaced as its own section.
...
cron_health AS (
  WITH critical_jobs AS (
    ...
    FROM cron.job
    WHERE active = true AND jobname NOT ILIKE '%stripe%' AND jobname NOT ILIKE '%partner%'
  ),
  last_runs AS (
    SELECT jobid,
...
cron_summary AS (
  SELECT
    ...
    count(*) FILTER (WHERE health='NEWLY_SCHEDULED') AS newly_scheduled,
    ...
    count(*) AS total,
    string_agg(jobname, ', ') FILTER (WHERE health NOT IN ('HEALTHY','NEWLY_SCHEDULED')) AS unhealthy_jobs
  FROM cron_health
),
pipeline_lag AS (
  SELECT
    (SELECT count(*) FROM warm_outbound_staging WHERE enrichment_status='enriched' AND email IS NOT NULL AND fit_score IS NULL AND enriched_at < now() - interval '2 hours') AS fit_score_lag,
    (SELECT count(*) FROM warm_outbound_staging WHERE enrichment_status='pending' AND created_at < now() - interval '4 hours') AS enrichment_lag,
    (SELECT count(*) FROM warm_outbound_staging WHERE fit_score>=80 AND first_touch_draft_body IS NULL AND fit_scored_at IS NOT NULL AND fit_scored_at < now() - interval '24 hours' AND automation_paused=false AND is_paying_customer IS NOT TRUE AND replied_at IS NULL AND dispatched_at IS NULL AND archived_at IS NULL) AS draft_lag,
    (SELECT count(*) FROM warm_outbound_staging WHERE first_touch_draft_body IS NOT NULL AND dispatched_at IS NULL AND first_touch_drafted_at < now() - interval '24 hours' AND automation_paused=false AND is_paying_customer IS NOT TRUE AND replied_at IS NULL AND archived_at IS NULL) AS send_lag
),
throughput AS (
  SELECT
    ...
    (SELECT count(*) FROM gmail_send_log WHERE created_at > now() - interval '24 hours') AS last_24h
),
...
conversion_snapshot AS (
  SELECT captured_at,
    case_file_views_unique, lock_views_unique,
    email_gate_shown_unique, email_gate_submitted, inline_gate_conversion_pct,
    universal_gate_shown, universal_gate_submitted, universal_gate_conversion_pct,
    main_gate_shown, main_gate_submitted, main_gate_conversion_pct,
    purchases_total, email_captures_total, lead_enrichment_failed,
    delta_vs_prior,
    reconstruction_pageviews_unique,
    reconstruction_terms_accepted,
    reconstruction_decision_selected,
    reconstruction_capa_started,
    reconstruction_coming_soon_shown,
    reconstruction_completed,
    reconstruction_routed,
    reconstruction_completion_pct,
    reconstruction_route_pct,
    reconstruction_dead_dropdown_pct,
    reconstruction_observation_breakdown
...
playbook_latest AS (
  ...
  WHERE status='pending' ORDER BY generated_at DESC LIMIT 1
)
SELECT
  '=== CW SESSION BRIEFING ' || to_char((now() AT TIME ZONE 'America/New_York'), 'YYYY-MM-DD HH24:MI EDT') || ' ===' ||
  E'\n\nREVENUE\n' ||
  '  May MTD: $' || COALESCE(round(r.may_revenue_usd, 2)::text, '0.00') ||
  ' of $' || round(r.may_target_usd)::text ||
  ' (' || round(r.pct_to_target, 1)::text || '%)' ||
  E'\n  Days remaining: ' || r.days_remaining_to_may31::text ||
  ' | Required daily run rate: $' || round(r.required_daily_run_rate_usd, 2)::text ||
  E'\n\nCONVERSION (last 7d, PostHog)\n' ||
  CASE WHEN cs.captured_at IS NULL THEN E'  No snapshot yet — first daily run at 5:15 AM EDT\n\n'
  ELSE
    '  Case file unique visitors: ' || cs.case_file_views_unique::text ||
    ...
    E'\n  Snapshot age: ' || round(EXTRACT(epoch FROM now() - cs.captured_at) / 3600, 1)::text || 'h' ||
    E'\n\n'
  END ||
  -- v6: RECONSTRUCTION FUNNEL section. Shows actual /reconstruction conversion data.
  CASE WHEN cs.captured_at IS NULL OR cs.reconstruction_pageviews_unique IS NULL THEN ''
  ELSE
    E'RECONSTRUCTION FUNNEL (/reconstruction, last 7d)\n' ||
    '  Visitors: ' || cs.reconstruction_pageviews_unique::text ||
    ' → Terms accepted: ' || COALESCE(cs.reconstruction_terms_accepted::text,'0') ||
    ' → Decision picked: ' || COALESCE(cs.reconstruction_decision_selected::text,'0') ||
    E'\n  CAPA path started: ' || COALESCE(cs.reconstruction_capa_started::text,'0') ||
    ' | Dead-end (coming soon): ' || COALESCE(cs.reconstruction_coming_soon_shown::text,'0') ||
    ' (' || COALESCE(cs.reconstruction_dead_dropdown_pct::text,'0') || '% of pickers)' ||
    E'\n  Completed all 8: ' || COALESCE(cs.reconstruction_completed::text,'0') ||
    ' (' || COALESCE(cs.reconstruction_completion_pct::text,'0') || '% of visitors)' ||
    E'\n  Routed to case file: ' || COALESCE(cs.reconstruction_routed::text,'0') ||
    ' (' || COALESCE(cs.reconstruction_route_pct::text,'0') || '% of visitors)' ||
    CASE WHEN cs.reconstruction_observation_breakdown IS NOT NULL
              AND jsonb_array_length(cs.reconstruction_observation_breakdown) > 0
         THEN E'\n  Top observation: ' || (cs.reconstruction_observation_breakdown->0->>'key') ||
              ' (' || (cs.reconstruction_observation_breakdown->0->>'n') || ' completers)'
         ELSE ''
    END ||
    E'\n\n'
  END ||
  CASE WHEN pl_latest.id IS NOT NULL
       THEN 'PLAYBOOK DECISION (pending sign-off)' || E'\n  ' || pl_latest.recommendation || E'\n\n'
       ELSE ''
  END ||
  ...
  E'\nGMAIL AUTH\n' ||
  CASE WHEN gah.last_successful_auth IS NULL THEN E'  ⚠ No auth attempts logged yet (table newly created — first batch will populate)\n'
  ELSE
    '  Last successful refresh: ' || to_char((gah.last_successful_auth AT TIME ZONE 'America/New_York'),'Mon DD HH24:MI') ||
    ' (' || round(gah.days_since_last_success, 1)::text || ' days ago)' ||
    ...
    E'\n' ||
    CASE WHEN gah.days_since_last_success > 150 THEN E'  ⚠ Token age warning: > 150 days (Google revokes at 180)\n' ELSE '' END
  END ||
  ...
  ' (' || EXTRACT(day FROM li.days_since_intake)::integer::text || ' days ago)' ||
  E'\n'
  AS briefing
FROM revenue r,
