-- Smarter cron detector: flag only when last_run is null AND job is older than 24h, OR last_run failed.
-- Excludes legitimately infrequent jobs (monthly, weekly, quarterly).
...
crons AS (
  -- Only flag frequent crons (interval/hour/few times daily) that haven't succeeded recently.
  -- Excludes weekly, monthly, quarterly.
  SELECT count(*) AS broken_crons
  FROM cron.job j
  WHERE j.active = true
    AND (j.schedule LIKE '*/%' OR j.schedule LIKE '%* * * *' OR j.schedule LIKE '% *-* * * *' OR j.schedule ~ '^[0-9,]+( [0-9,*/-]+){4}$')
    AND j.schedule NOT LIKE '% * * 1' -- not weekly Mondays
    AND j.schedule NOT LIKE '% % 1 % %' -- not monthly 1st
    AND EXISTS (SELECT 1 FROM cron.job_run_details d WHERE d.jobid = j.jobid)
    AND NOT EXISTS (SELECT 1 FROM cron.job_run_details d WHERE d.jobid = j.jobid AND d.start_time > NOW() - INTERVAL '6 hours' AND d.status = 'succeeded')
),
phantoms AS (
  SELECT count(*) AS phantom_count
  FROM warm_outbound_staging ws
  WHERE ws.dispatched_at IS NOT NULL AND ws.dispatched_at > NOW() - INTERVAL '7 days'
...
SELECT 
  CONCAT(
    ...
    CASE WHEN crons.broken_crons > 0 THEN '⚠ ' || crons.broken_crons || E' frequent crons not succeeded in 6h\n' ELSE '' END,
