...
BEGIN
  ...
  -- Map amount to artifact_id
  v_artifact_id := CASE
    WHEN v_amount = 2700  THEN 'decision-authority-matrix'
    WHEN v_amount = 29700 THEN 'authorization-package'
    ELSE NULL
  END;
  ...
  -- Write to artifact_access for gated products
  IF v_artifact_id IS NOT NULL AND v_email IS NOT NULL AND v_email != '' THEN
    INSERT INTO public.artifact_access (
      email,
      artifact_id,
      stripe_session_id,
...
-- Manually grant Mansoor access since he already paid
INSERT INTO public.artifact_access (
  email,
  artifact_id,
  stripe_session_id,
  ...
  'mansur2191@gmail.com',
  'decision-authority-matrix',
  'cs_live_a1Oc97UGv2jZ4vgmayUyhbbzRY9N2w29Ml9QPvPbdU9qEIwftpIOMvkorR',
  'pi_3TJ1IIBcdOgm3yGB0J1tc0fJ',
  true,
  now()
)
ON CONFLICT DO NOTHING;
