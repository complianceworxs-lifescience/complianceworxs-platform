...
BEGIN
  ...
  -- NEW: when drafter finishes (first_touch_drafted_at stamped), fire verifier
  IF NEW.first_touch_drafted_at IS NOT NULL AND OLD.first_touch_drafted_at IS NULL THEN
    ...
    IF COALESCE(NEW.automation_paused, false) = false
       ...
       AND NEW.email_verification_status IS NULL THEN
      BEGIN
        SELECT net.http_get(
          url := v_base_url || '/email-verifier?id=' || NEW.id || '&secret=' || v_secret,
          timeout_milliseconds := 60000
        ) INTO v_request_id;
      EXCEPTION WHEN OTHERS THEN
        INSERT INTO outbound_events (staging_id, event_name, provider, properties)
        VALUES (NEW.id, 'cascade_dispatch_failed', 'trigger',
          jsonb_build_object('stage', 'email_verifier', 'error', SQLERRM));
      END;
    END IF;
  END IF;

  -- NEW: when verifier passes (email_verification_status set, no suppression), auto-approve to fire sender
  IF NEW.email_verification_status IS NOT NULL AND OLD.email_verification_status IS NULL THEN
    INSERT INTO outbound_events (staging_id, attio_record_id, email, event_name, provider, properties)
    VALUES (
      NEW.id, NEW.attio_record_id, NEW.email, 'lead_email_verified', 'hunter',
      v_industry_props || jsonb_build_object(
        'full_name', NEW.full_name,
        'verification_status', NEW.email_verification_status,
        'verification_score', NEW.email_verification_score,
        'blocked', NEW.suppression_reason IS NOT NULL
      )
    );

    IF NEW.suppression_reason IS NULL
       AND COALESCE(NEW.email_approved, false) = false
       AND NEW.first_touch_draft_body IS NOT NULL THEN
      UPDATE warm_outbound_staging
      SET email_approved = true,
          email_approved_at = now(),
          email_approved_by = 'auto_verifier_v1'
      WHERE id = NEW.id;
    END IF;
  END IF;
  ...
  IF NEW.email_approved = true AND COALESCE(OLD.email_approved, false) = false THEN
    IF COALESCE(NEW.automation_paused, false) = false
       ...
       AND NEW.first_touch_draft_body IS NOT NULL
       AND NEW.suppression_reason IS NULL THEN
      BEGIN
        SELECT net.http_get(
