...
DROP VIEW IF EXISTS warm_leads_dm_list;

CREATE VIEW warm_leads_dm_list AS
...
scored AS (
  ...
  WHERE (
    ls.cta_clicks > 0 OR
    ls.checkout_attempts > 0 OR
    ls.return_visits > 0 OR
    ls.lock_views >= 2
  )
),
enriched AS (
  SELECT
    ...
    s.exit_intents,
    s.last_seen,
    COALESCE(l.email, sess.email)                AS email,
    COALESCE(l.name,  sess.identified_company)   AS name,
    l.title,
    COALESCE(l.company, sess.identified_company) AS company,
    sess.identified_company                      AS identified_company,
  ...
  LEFT JOIN purchases p       ON p.email         = COALESCE(l.email, sess.email)
  LEFT JOIN outreach_queue oq ON oq.email        = COALESCE(l.email, sess.email)
  WHERE p.id IS NULL
)
SELECT
  warmth_score,
  COALESCE(name, identified_company, 'Unknown')   AS name,
  COALESCE(email, 'No email captured')            AS email,
  COALESCE(title, '—')                            AS title,
  COALESCE(company, identified_company, '—')      AS company,
  identified_company,
  case_file,
  return_visits,
  cta_clicks,
  ...
  CASE
    ...
    WHEN return_visits > 0
     AND lock_views >= 2        THEN 'Hit the paywall on a return visit — price or trust gap'
