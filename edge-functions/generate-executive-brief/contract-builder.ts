// contract-builder.ts
//
// Builds one Editorial Contract for the Executive Brief asset type.
// Every design choice below is a real product decision, not a
// placeholder -- stated in the README rather than left implicit.
...
export function buildExecutiveBriefContract(input: ExecutiveBriefRequest) {
  ...
  return {
    ...
    constraints: [
      ...
      'No more than five sections.',
      `Source material to draw from: ${input.sourceMaterial}`,
    ],
    acceptanceCriteria: [
      'States the recommendation before background elaboration.',
      'Contains exactly one primary recommendation.',
      'Names the specific decision or gap the audience must act on.',
    ],
