...
import { validateBatchReviewRequest } from './service.ts';
import { runBatchReviewPipeline } from './pipeline.ts';
...
serve(async (req) => {
  ...
  const validation = validateBatchReviewRequest(body);
  if (!validation.valid) {
    return jsonResponse({ status: 'rejected', stage: 'request_invalid', issues: validation.issues }, 400);
  }

  const result = await runBatchReviewPipeline(validation.request, urls, fetch);
