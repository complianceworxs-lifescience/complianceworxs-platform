import { BatchReviewRequest, PipelineResult, PipelineUrls } from './types.ts';
import { buildBatchReviewContract } from './contract-builder.ts';
...
export async function runBatchReviewPipeline(input: BatchReviewRequest, urls: PipelineUrls, fetchImpl: typeof fetch): Promise<PipelineResult> {
  const contract = buildBatchReviewContract(input);
  ...
  const runtimeResult = await callJson(urls.runtime, { promptPackage, userVariables: { decisionDomain: input.decisionDomain, submittedMaterial: input.submittedMaterial } }, fetchImpl);
  ...
  return buildCompletedResponse({
    ...
    submittedDocumentCount: input.submittedDocuments.length,
    artifact, executionSpecification, promptPackage, runtimeManifest,
