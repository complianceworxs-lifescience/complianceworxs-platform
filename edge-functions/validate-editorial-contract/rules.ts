// rules.ts — compiled artifact, never parse the source document at runtime.

export type RuleCategory =
  | 'EP' | 'EV' | 'RG' | 'RS' | 'ES' | 'NP' | 'EC' | 'AS'
  | 'EDE' | 'EQS' | 'EG' | 'AIB'
  | 'INV' | 'FM' | 'AC' | 'QG' | 'CL' | 'STATE';
...
export const CHAPTER_TITLES: Record<number, string> = {
  ...
  7: 'Editorial Contracts',
  8: 'Asset Contracts',
  9: 'Editorial Decision Engine',
  10: 'Editorial Quality System',
  11: 'Editorial Governance',
  12: 'AI Implementation Boundary',
  13: 'Appendices',
};

function ids(prefix: string, count: number, pad = 3): string[] {
  return Array.from({ length: count }, (_, i) => `${prefix}-${String(i + 1).padStart(pad, '0')}`);
}

function idsNoSep(prefix: string, count: number, pad = 3): string[] {
  return Array.from({ length: count }, (_, i) => `${prefix}${String(i + 1).padStart(pad, '0')}`);
}
...
for (const [id, category, chapter] of registryRows) {
  RULE_REGISTRY[id] = { id, category, chapter, chapterTitle: CHAPTER_TITLES[chapter] };
}

export function ruleExists(id: string): boolean {
  return Object.prototype.hasOwnProperty.call(RULE_REGISTRY, id);
}

export function ruleCategory(id: string): RuleCategory | null {
  return RULE_REGISTRY[id]?.category ?? null;
}

export function ruleBelongsToChapter(id: string, chapter: number): boolean {
  return RULE_REGISTRY[id]?.chapter === chapter;
}
